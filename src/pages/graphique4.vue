...
const filteredNeigeData = computed(() => {
  return neigeData25.value.filter(d => 
  ...
  marks: [
    ...
    ...(filteredNeigeData.value.length > 0 ? [
      Plot.dot(filteredNeigeData.value, {
        x: 'longitude',
        y: 'latitude',
        fill: d => d.NBJNEIGETOT1 > 30 ? "#001f4d" : d.NBJNEIGETOT1 > 10 ? "#4d79ff" : "#b3c6ff",
...
<template>
  <h1>Visualisation de l'enneigement dans le Doubs</h1>
  ...
  <select v-model="selectedMonthNeige" id="month-select">
  ...
  <div class="container mx-auto p-4">
    ...
    <p class="mb-4">
      Le graphique présente une visualisation des jours d'enneigement dans le Département 25 (Doubs) pour des périodes spécifiques, en comparant le nombre de jours avec plus de 1 cm, 10 cm, et 30 cm de neige. La carte géographique illustre également la répartition géographique des jours d'enneigement à l'aide de points codés par couleur et taille.
    </p>
    <h1 class="text-4xl font-bold text-center mb-6">Analyse</h1>
    <p class="mb-4">
      <strong>Les valeurs observées :</strong> Le graphique montre le nombre total de jours d'enneigement pour le Département 25. Par exemple, il est possible d'observer un nombre significatif de jours avec plus de 1 cm de neige, représentant une base de l'analyse. Les jours avec plus de 10 cm et 30 cm de neige sont également présentés, permettant de visualiser la variabilité de l'enneigement.
    </p>
    <p class="mb-4">
      <strong>Visualisation :</strong> La carte utilise une palette de couleurs dégradées allant du bleu clair au bleu foncé, indiquant des niveaux d'enneigement différents. Les points représentant les jours d'enneigement sont agrandis pour plus de visibilité, et chaque point est entouré d'un contour blanc pour un meilleur contraste. Les annotations sur la carte fournissent une compréhension immédiate des jours d'enneigement, avec des chiffres affichés au-dessus des points, ce qui facilite l'interprétation des données.
    </p>
    <p class="mb-4">
      Dans le cadre du projet visant à analyser les changements climatiques en France, cette visualisation des jours d'enneigement permet aux citoyens et aux décideurs de mieux comprendre les dynamiques locales liées à l'enneigement. Cela aide à contextualiser comment le changement climatique pourrait affecter la fréquence et l'intensité des épisodes neigeux, des informations essentielles pour planifier les stratégies d'adaptation aux changements climatiques à l'échelle régionale.
    </p>
